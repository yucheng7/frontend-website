...
const getSearchAnimeList = async () => {
    try {
        const res = await axios.get(`https://api.annict.com/v1/works?access_token=C23CjuV8eGIYLnn0qRkUUhDTWdl6KFwuS-ZzzTy9IB0&filter_title=${animeTitle.value}`)
    ...
    <div class="container">
        ...
        <div class="animelist-search">
            ...
            <div class="animelist-search-select-bar">
                <input type="text" class="animelist-search-input" placeholder="輸入日文作品名搜尋" value="" v-model="animeTitle"
                    @input="handleSearchChange" v-show="searchType == 1">
        ...
        <div class="animelist-content" v-if="searchType == 0">
            <div class="animelist-content-title">這是animelist搜尋結果，{{ filterYear }}年{{ seasonChinese }}總共{{ data.length
                }}部
            </div>
